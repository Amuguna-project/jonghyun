$(function(){
    ...
    $('#button').on('click', function(){
        ...
        widthNode = member;                 // widthNode : 인원수 
        setTimeout(function(){
            $('#landing').remove();         // #landing 페이지 삭제 
            init();
        }, 310)                          // 0.310초 동안 setTimeout 

    });

    function canvasDraw(){               // 1번째로 실행되는 함수 
        ladder.css({                     // ladder div 생성 
            'width' :( widthNode-1) * 100 + 6,
            'height' : (heightNode -1 ) * 25 + 6,
            'background-color' : '#fff'
        });
       ladder_canvas                   //ladder_canvas 생성
       .attr('width' , ( widthNode-1) * 100 + 6)
       .attr('height' , ( heightNode-1) * 25 + 6);

        setDefaultFootPrint();        // 332.

        reSetCheckFootPrint();        // 341.
    ...
    function setDefaultFootPrint(){               // 2번쨰로 실행되는 함수
    ...
    function reSetCheckFootPrint(){            // 3번째로 실행되는 함수
